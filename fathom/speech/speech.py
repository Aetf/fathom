...
class Speech(NeuralNetworkModel):
    ...
    def bidirectional_layer(self, inputs, n_input, n_hidden, n_output):
        ...
        with self.G.as_default():
            ...
            inputs = tf.split(axis=0, num_or_size_splits=self.max_frames,
                              value=inputs)  # max_time * (batch_size, n_hidden)
            ...
            outputs, _, _ = tf.contrib.rnn.static_bidirectional_rnn(
                fw_cell, bw_cell, inputs, initial_state_fw=istate_fw, initial_state_bw=istate_bw)
