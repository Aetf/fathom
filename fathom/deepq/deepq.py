...
class DeepQNetNature(object):
    ...
    def build(self, params):
        with self.G.as_default():
            ...
            self.w1 = tf.Variable(tf.random_normal([size, size, channels, filters],
                                                   stddev=0.01), name=self.network_name + '_'+layer_name+'_weights')
            ...
            self.c1 = tf.nn.conv2d(self.x, self.w1, strides=[
                                   1, stride, stride, 1], padding='VALID', name=self.network_name + '_'+layer_name+'_convs')
            ...
            filters = 64
            stride = 1
            self.w3 = tf.Variable(tf.random_normal([size, size, channels, filters],
                                                   stddev=0.01), name=self.network_name + '_'+layer_name+'_weights')
            self.b3 = tf.Variable(tf.constant(0.1, shape=[filters]), name=self.network_name + '_'+layer_name+'_biases')
            self.c3 = tf.nn.conv2d(self.o2, self.w3, strides=[
                                   1, stride, stride, 1], padding='VALID', name=self.network_name + '_'+layer_name+'_convs')
    ...
    def train(self):
        with self.G.as_default():
            ...
            self.rmsprop = tf.train.RMSPropOptimizer(self.params['lr'], self.params['rms_decay'], 0.0, self.params['rms_eps']).minimize(
...
class DeepQ(GenericModel):
    ...
    def __init__(self, device=None, init_options=None, game=nature_params['game']):
        ...
        self.engine = emulator(rom_name='{}.bin'.format(
            game), vis=self.params['visualize'], frameskip=self.params['frameskip'], windowname=self.params['window_name'])
    ...
    def setup(self, setup_options=None):
        ...
        with self.G.as_default():
            if setup_options is None:
                self.setup_config = tf.ConfigProto(gpu_options=tf.GPUOptions(
                    per_process_gpu_memory_fraction=self.params['gpu_fraction']))
            else:
        ...
        with self.G.as_default():
            while self.step < (self.params['steps_per_epoch'] * self.params['num_epochs'] * self.params['learning_interval'] + self.params['train_start']):
                ...
                if not self.forward_only and self.step % self.params['learning_interval'] == 0 and self.DB.get_size() > self.params['train_start']:
                    ...
                    _, self.train_cnt, self.cost = runstep(
                        self.sess, [self.qnet.rmsprop, self.qnet.global_step, self.qnet.cost], feed_dict=feed_dict)

                    self.total_cost_train += np.sqrt(self.cost)
                    self.train_cnt_for_disp += 1

                if not self.forward_only:
                    self.params['eps'] = max(self.params['eps_min'], 1.0 - float(self.train_cnt *
                                                                                 self.params['learning_interval'])/float(self.params['eps_step']))
                else:
                ...
                if not self.forward_only and self.step > 0 and self.step % self.params['eval_freq'] == 0 and self.DB.get_size() > self.params['train_start']:
                    self.reset_game()
                    if self.step % self.params['steps_per_epoch'] == 0:
                        self.reset_statistics('all')
                    else:
                ...
                if not self.forward_only:
                    ...
                    self.total_Q_train += self.maxQ
                else:
                    self.epi_reward_eval += self.reward
                    self.total_Q_eval += self.maxQ
